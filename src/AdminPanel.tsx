...
import { diagnoseSupabaseConnection, fetchRemoteCmsState, saveRemoteCmsState, subscribeToRemoteCms, uploadMediaToSupabase } from "./cmsSync";
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      if (hasSupabaseConfig) {
        ...
        const result = await saveRemoteCmsState(cms as unknown as Record<string, unknown>);
        remoteOk = result.ok;
        if (!result.ok) {
          remoteErrorMessage = "error" in result ? result.error ?? "Erreur inconnue" : "Supabase n'a pas répondu (vérifiez l'URL, la clé, et le script SQL).";
        }
      }
      ...
      if (!hasSupabaseConfig) {
        ...
        addToast("Sauvegarde locale effectuée (Supabase non configuré : les autres visiteurs ne verront pas ce changement).", "info");
      } else if (remoteOk) {
        setSyncStatus("success");
        addToast("Sauvegarde effectuée et partagée via Supabase.", "success");
      } else {
        setSyncStatus("error");
        setSyncErrorMessage(remoteErrorMessage);
        addToast(`Échec de la synchronisation Supabase : ${remoteErrorMessage}`, "error");
      }
    }, 2200);
  ...
  function deleteCollectionItem(kind: CollectionKind, itemId: string) {
    mutateCms(
      (draft) => {
        draft.collections[kind] = draft.collections[kind].filter((item) => item.id !== itemId);
        return draft;
      },
      "Élément supprimé définitivement.",
    );
    setSelectedCollectionIds((current) => current.filter((id) => id !== itemId));
  }
  ...
  async function registerUploadedFiles(files: FileList | null, preferredType?: MediaType) {
    ...
    const uploadedEntries = await Promise.all(
      Array.from(files).map(async (file) => {
        ...
        const remoteUpload = await uploadMediaToSupabase(file, inferredType === "document" ? "documents" : inferredType === "video" ? "videos" : "gallery");
        if (!remoteUpload.ok) {
          hadError = true;
          lastErrorMessage = remoteUpload.error;
        }
        ...
        return {
          ...
          src: remoteUpload.ok ? remoteUpload.publicUrl : URL.createObjectURL(file),
          size: (remoteUpload.ok ? remoteUpload.size : undefined) || file.size || 1024,
    ...
    mutateCms(
      ...
      `${files.length} média importé${files.length > 1 ? "s" : ""}.`,
    );

    if (hadError) {
      addToast(
        `Attention : l'envoi vers Supabase a échoué (${lastErrorMessage}). Le média n'est visible que sur ce navigateur, pas pour les autres visiteurs.`,
        "error",
      );
      setSyncStatus("error");
      setSyncErrorMessage(lastErrorMessage);
    } else if (hasSupabaseConfig) {
      addToast("Média envoyé vers Supabase Storage avec succès.", "success");
    }
  }

  async function replaceMedia(mediaId: string, file: File | null) {
    if (!file) return;
    const remoteUpload = await uploadMediaToSupabase(
      file,
        ...
        draft.media = draft.media.map((item) =>
          item.id === mediaId
            ? {
                ...
                name: file.name,
                src: remoteUpload.ok ? remoteUpload.publicUrl : URL.createObjectURL(file),
                size: (remoteUpload.ok ? remoteUpload.size : undefined) || file.size || item.size,
                createdAt: new Date().toISOString(),
    ...
    if (!remoteUpload.ok) {
      addToast(`Attention : l'envoi vers Supabase a échoué (${remoteUpload.error}). Le média n'est visible que sur ce navigateur.`, "error");
      setSyncStatus("error");
      setSyncErrorMessage(remoteUpload.error);
    } else if (hasSupabaseConfig) {
      addToast("Média remplacé et envoyé vers Supabase avec succès.", "success");
    }
  }
  ...
  async function uploadAndAttachMedia(kind: CollectionKind, itemId: string, file: File | null, preferredType?: MediaType) {
    ...
    mutateCms(
      ...
      "Média importé et associé à l'élément.",
    );

    if (!remoteUpload.ok) {
      addToast(`Attention : l'envoi vers Supabase a échoué (${remoteUpload.error}). Ce média ne sera pas visible pour les autres visiteurs.`, "error");
      setSyncStatus("error");
      setSyncErrorMessage(remoteUpload.error);
    } else if (hasSupabaseConfig) {
      addToast("Média envoyé vers Supabase et associé avec succès.", "success");
    }
  }
  ...
  async function importMediaIntoCollection(kind: CollectionKind, files: FileList | null, preferredType: MediaType) {
    ...
    const entries = await Promise.all(
      Array.from(files).map(async (file) => {
        const remoteUpload = await uploadMediaToSupabase(file, preferredType === "video" ? "videos" : "gallery");
        if (!remoteUpload.ok) {
          hadError = true;
          lastErrorMessage = remoteUpload.error;
        }
    ...
    mutateCms(
      ...
      `${files.length} ${preferredType === "video" ? "vidéo(s)" : "photo(s)"} importée(s) dans ${kind}.`,
    );

    if (hadError) {
      addToast(
        `Attention : l'envoi vers Supabase a échoué (${lastErrorMessage}). Ces médias ne seront pas visibles pour les autres visiteurs.`,
        "error",
      );
      setSyncStatus("error");
      setSyncErrorMessage(lastErrorMessage);
    } else if (hasSupabaseConfig) {
      addToast("Médias envoyés vers Supabase avec succès.", "success");
    }
  }
  ...
  function deleteMediaItem(mediaId: string) {
    mutateCms(
      (draft) => {
        draft.media = draft.media.filter((item) => item.id !== mediaId);
        // On détache aussi ce média des blocs et collections qui l'utilisaient,
        // afin de ne jamais laisser une référence vers un média supprimé.
        draft.pages = draft.pages.map((page) => ({
          ...page,
          blocks: page.blocks.map((block) => (block.mediaId === mediaId ? { ...block, mediaId: undefined } : block)),
        }));
        (Object.keys(draft.collections) as CollectionKind[]).forEach((kind) => {
          draft.collections[kind] = draft.collections[kind].map((item) =>
            item.mediaId === mediaId ? { ...item, mediaId: undefined } : item,
          );
        });
        return draft;
      },
  ...
  async function forceSyncNow() {
    if (!hasSupabaseConfig) {
      setSyncStatus("not-configured");
      addToast("Supabase n'est pas configuré : ajoutez VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY, puis redéployez.", "error");
      return;
    }
    setSyncStatus("syncing");
    const result = await saveRemoteCmsState(cms as unknown as Record<string, unknown>);
    if (result.ok) {
      ...
      lastSnapshotRef.current = JSON.stringify(cms);
      addToast("Synchronisation manuelle réussie : vos changements sont maintenant partagés.", "success");
    } else {
      setSyncStatus("error");
      const message = "error" in result ? result.error ?? "Erreur inconnue" : "Impossible de contacter Supabase.";
      setSyncErrorMessage(message);
      addToast(`Échec de synchronisation : ${message}`, "error");
    }
  }

  async function testSupabaseConnection() {
    setSyncStatus("syncing");
    const diagnostic = await diagnoseSupabaseConnection();
    if (diagnostic.ok) {
      setSyncStatus("success");
      setSyncErrorMessage(null);
      addToast(diagnostic.message, "success");
    } else {
      setSyncStatus("error");
      setSyncErrorMessage(diagnostic.message);
      addToast(diagnostic.message, "error");
    }
  }
              ...
              <div className="flex flex-wrap items-center gap-3">
                <span
                  className={cn(
                    ...
                    !hasSupabaseConfig && "bg-amber-100 text-amber-800",
                    hasSupabaseConfig && syncStatus === "success" && "bg-emerald-100 text-emerald-800",
                    hasSupabaseConfig && syncStatus === "error" && "bg-rose-100 text-rose-800",
                    hasSupabaseConfig && (syncStatus === "idle" || syncStatus === "syncing") && (isDark ? "bg-white/6 text-slate-300" : "bg-slate-100 text-slate-600"),
                  )}
                  title={syncErrorMessage || undefined}
                >
                  <span className={cn("h-2 w-2 rounded-full", !hasSupabaseConfig ? "bg-amber-500" : syncStatus === "success" ? "bg-emerald-500" : syncStatus === "error" ? "bg-rose-500" : "bg-slate-400")} />
                  {!hasSupabaseConfig
                    ? "Supabase non configuré (visible sur ce navigateur seulement)"
                    : syncStatus === "syncing"
                ...
                <button type="button" onClick={() => void testSupabaseConnection()} className={cn("rounded-full border px-4 py-2 text-xs font-semibold", isDark ? "border-white/10 bg-white/5 text-white" : "border-slate-200 bg-white text-slate-700")}>
                  Tester la connexion
                </button>
                <button type="button" onClick={() => void forceSyncNow()} className={cn("rounded-full border px-4 py-2 text-xs font-semibold", isDark ? "border-white/10 bg-white/5 text-white" : "border-slate-200 bg-white text-slate-700")}>
...
function CollectionsTab({
  ...
  deleteCollectionItem: (kind: CollectionKind, itemId: string) => void;
}) {
                ...
                <button
                  ...
                  onClick={() => {
                    if (window.confirm("Supprimer définitivement cet élément ?")) {
                      deleteCollectionItem(selectedCollectionKind, item.id);
                    }
                  }}
                  className={dangerIconButton(isDark)}
...
function SecurityTab({
  ...
  setChangeCodeForm: (value: { current: string; next: string; confirm: string }) => void;
  changeAdminCode: () => Promise<void>;
  syncStatus: "idle" | "syncing" | "success" | "error" | "not-configured";
  syncErrorMessage: string | null;
  testSupabaseConnection: () => Promise<void>;
  forceSyncNow: () => Promise<void>;
}) {
  const configuredUrl = (import.meta.env.VITE_SUPABASE_URL as string | undefined) || null;
  const configuredBucket = (import.meta.env.VITE_SUPABASE_BUCKET as string | undefined) || "site-media";
  ...
  return (
    <>
      ...
      <AdminCard isDark={isDark} className="mb-6 p-6 sm:p-7">
        <SectionHeader
          title="Diagnostic Supabase"
          description="Vérifiez pourquoi vos suppressions ou vos médias ne sont pas visibles par tout le monde."
          isDark={isDark}
        />
        <div className="grid gap-4 sm:grid-cols-2">
          <div className={cn("rounded-[22px] border p-4", isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50")}>
            <p className="text-sm font-semibold">Variables détectées au build</p>
            <p className={cn("mt-2 break-all text-sm leading-7", isDark ? "text-slate-300" : "text-slate-600")}>
              URL : {configuredUrl || "absente"}
              <br />
              Bucket : {configuredBucket}
            </p>
            {!hasSupabaseConfig ? (
              <p className="mt-3 rounded-xl bg-amber-100 px-3 py-2 text-xs font-semibold text-amber-800">
                Aucune variable détectée. Ajoutez VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY dans Vercel (Settings → Environment Variables), puis redéployez.
              </p>
            ) : null}
          </div>
          <div className={cn("rounded-[22px] border p-4", isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50")}>
            <p className="text-sm font-semibold">État actuel</p>
            <p className={cn("mt-2 text-sm leading-7", isDark ? "text-slate-300" : "text-slate-600")}>
              {syncStatus === "syncing"
                ...
                : syncStatus === "success"
                  ...
                  : syncStatus === "error"
                    ? `Erreur : ${syncErrorMessage || "inconnue"}`
                    : "Aucun test effectué pour le moment."}
            </p>
          </div>
        </div>
        <div className="mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={() => void testSupabaseConnection()} className={primaryAction(isDark)}>
            Tester la connexion Supabase
          </button>
          <button
            type="button"
            onClick={() => void forceSyncNow()}
            className={cn("rounded-full border px-5 py-3 text-sm font-semibold", isDark ? "border-white/10 bg-white/5 text-white" : "border-slate-200 bg-white text-slate-800")}
          >
            Forcer l'envoi des données actuelles
          </button>
        </div>
        <div className={cn("mt-5 rounded-[22px] border p-4 text-sm leading-7", isDark ? "border-white/10 bg-white/5 text-slate-300" : "border-slate-200 bg-slate-50 text-slate-600")}>
          <p className="font-semibold">Si vos suppressions ou médias ne se voient pas ailleurs :</p>
          <ol className="mt-2 list-decimal space-y-1 pl-5">
            <li>Vérifiez que les 4 variables sont bien ajoutées dans Vercel, sur l'environnement <strong>Production</strong>.</li>
            <li>Vérifiez que vous avez bien redéployé <strong>après</strong> avoir ajouté les variables (elles ne s'appliquent pas rétroactivement).</li>
            <li>Vérifiez que le script SQL a été exécuté sur ce même projet Supabase (table <code>site_state</code> + bucket <code>site-media</code>).</li>
            <li>Cliquez sur « Tester la connexion Supabase » ci-dessus pour confirmer.</li>
          </ol>
        </div>
      </AdminCard>
