...
function HomePage() {
  ...
  // Important : dès que le CMS a chargé une vraie structure (même vide après
  // suppression par l'administrateur), on ne doit plus jamais revenir aux
  // contenus statiques de démonstration. Sinon une suppression admin
  // "réapparaîtrait" pour les visiteurs.
  const cmsReady = Boolean(runtimeCms && runtimeCms.collections);
    ...
    <>
      ...
      <section className="section-space pt-0">
        <div className="section-shell">
                ...
                >
                  <div className="relative h-60 overflow-hidden">
                    ...
                    <div className="absolute left-4 top-4 rounded-full bg-white/85 px-3 py-1 text-[0.7rem] font-bold uppercase tracking-[0.22em] text-[#0E2A7B] backdrop-blur-sm">
                      {cmsReady ? "Depuis l'admin" : "Visuel temporaire"}
                    </div>
                  </div>
                  <div className="p-6">
          ...
          {cmsReady && runtimeFormations.length === 0 ? (
            <p className="mt-6 rounded-[24px] border border-dashed border-slate-300 bg-white/60 p-6 text-center text-sm text-slate-500">
              Aucune formation publiée pour le moment. Ajoutez-en depuis l'espace administrateur.
            </p>
          ) : null}
        </div>
      </section>
      ...
      <section className="section-space pt-0">
        <div className="section-shell">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {(cmsReady
              ? (runtimeCms?.media ?? [])
                  .filter((item) => !item.hidden && item.type !== "document")
                  .slice(0, 4)
                  .map((item) => ({
                    id: item.id,
          ...
          {cmsReady && (runtimeCms?.media ?? []).filter((item) => !item.hidden && item.type !== "document").length === 0 ? (
            <p className="mt-6 rounded-[24px] border border-dashed border-slate-300 bg-white/60 p-6 text-center text-sm text-slate-500">
              Aucun média publié pour le moment. Ajoutez des photos ou vidéos depuis l'espace administrateur.
            </p>
          ) : null}
        </div>
      </section>
          ...
          <Reveal>
            <div className="glass-panel rounded-[32px] p-7 sm:p-9">
              ...
              <div className="space-y-5">
                ...
                {cmsReady && runtimeNews.length === 0 ? (
                  <p className="rounded-[22px] border border-dashed border-slate-300 bg-white/60 p-5 text-center text-sm text-slate-500">
          ...
          <Reveal>
            <div className="glass-panel rounded-[32px] p-7 sm:p-9">
              ...
              <div className="space-y-5">
                ...
                {cmsReady && runtimeEvents.length === 0 ? (
                  <p className="rounded-[22px] border border-dashed border-slate-300 bg-white/60 p-5 text-center text-sm text-slate-500">
      ...
      <section className="section-space">
                ...
                <div className="relative min-h-[280px] lg:min-h-full">
                  ...
                  <div className="absolute inset-x-4 top-4 flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-[#081535]/72 px-3 py-1 text-[0.7rem] font-bold uppercase tracking-[0.22em] text-white backdrop-blur-sm">
                      {cmsReady ? "Depuis l'admin" : "Illustration temporaire"}
                    </span>
                  </div>
                </div>
        ...
        {cmsReady && runtimeFormations.length === 0 ? (
          <div className="section-shell mt-8">
            <p className="rounded-[24px] border border-dashed border-slate-300 bg-white/60 p-6 text-center text-sm text-slate-500">
              Aucune formation publiée pour le moment. Ajoutez-en depuis l'espace administrateur.
            </p>
          </div>
        ) : null}
...
function GalleryPage() {
  ...
  // Important : dès que le CMS a réellement chargé (même avec 0 média après
  // suppression par l'administrateur), on ne doit plus revenir aux photos
  // statiques de démonstration, sinon les suppressions "réapparaîtraient".
  const cmsReady = Boolean(runtimeCms && runtimeCms.media);
  ...
  const gallerySource = cmsReady ? runtimeGalleryItems : galleryItems;
    ...
    <>
      ...
      <section className="section-space">
        <div className="section-shell">
          ...
          {cmsReady && gallerySource.length === 0 ? (
            <p className="mt-6 rounded-[24px] border border-dashed border-slate-300 bg-white/60 p-6 text-center text-sm text-slate-500">
              Aucun média publié pour le moment. Ajoutez des photos ou vidéos depuis l'espace administrateur.
            </p>
          ) : null}
    ...
    <>
      ...
      <section className="section-space">
        ...
        {cmsReady && runtimeNews.length === 0 ? (
          <div className="section-shell mt-8">
            <p className="rounded-[24px] border border-dashed border-slate-300 bg-white/60 p-6 text-center text-sm text-slate-500">
              Aucune actualité publiée pour le moment. Ajoutez-en depuis l'espace administrateur.
            </p>
          </div>
        ) : null}
      </section>
        ...
        <div className="section-shell grid gap-8 xl:grid-cols-[1.05fr_0.95fr]">
          <Reveal>
            {(() => {
              ...
              if (cmsReady && !heroEvent) {
                return (
                  <div className="flex h-full min-h-[220px] items-center justify-center rounded-[34px] border border-dashed border-slate-300 bg-white/60 p-8 text-center text-sm text-slate-500">
                    Aucun événement publié pour le moment. Ajoutez-en depuis l'espace administrateur.
                  </div>
                );
              }
      ...
      <section className="section-space pt-0">
        ...
        {cmsReady && runtimeEvents.length === 0 ? (
          <p className="mt-6 text-center text-sm text-slate-500">
            Aucun autre événement publié pour le moment.
          </p>
        ) : null}
      </section>
    </>
